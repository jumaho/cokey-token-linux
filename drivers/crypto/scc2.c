 ...
 */
/*
 * Ported to mainline kernel by Julian Horsch, Fraunhofer AISEC, Germany
 */
...
/** Pointer to SCC's clock information.  Initialized during probe and scc_init(). */
static struct clk *scc_clk_ipg = NULL;
static struct clk *scc_clk_ahb = NULL;
...
static int scc_init(void)
{
	...
	if (scc_availability == SCC_STATUS_INITIAL) {
		...
		clk_prepare_enable(scc_clk_ipg);
		clk_prepare_enable(scc_clk_ahb);
	...
	/*
	clk_disable_unprepare(scc_clk_ipg);
	clk_disable_unprepare(scc_clk_ahb);
	*/
...
static void scc_init_iram(void)
{
	uint32_t reg_value;
	uint32_t reg_mask = 0;
	uint32_t partition_no;
	...
	scc_base = ioremap(scc_phys_base, 0x140);
	if (scc_base == NULL) {
	...
	scm_ram_base = ioremap(scm_ram_phys_base, ram_size);
...
static int scc_dev_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "got scc_phys_base: %x", r->start);
	...
	dev_err(&pdev->dev, "got scm_ram_start: %x", r->start);
	...
	irq_smn = platform_get_irq(pdev, 0);
	...
	irq_scm = platform_get_irq(pdev, 1);

	/* get clocks */
	scc_clk_ipg = devm_clk_get(&pdev->dev, "ipg");
	if (IS_ERR(scc_clk_ipg)) {
		dev_err(&pdev->dev, "Could not get ipg clock\n");
		ret = PTR_ERR(scc_clk_ipg);
		goto exit;
	}
	scc_clk_ahb = devm_clk_get(&pdev->dev, "ahb");
	if (IS_ERR(scc_clk_ahb)) {
		dev_err(&pdev->dev, "Could not get ahb clock\n");
		ret = PTR_ERR(scc_clk_ahb);
		goto exit;
	}
...
static struct platform_device_id scc_platform_ids[] = {
	{ .name = "imx53-scc" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(platform, scc_platform_ids);

static struct of_device_id scc_dt_ids[] = {
	{ .compatible = "fsl,imx53-scc2" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, scc_dt_ids);

static struct platform_driver imxscc_driver = {
	.driver = {
		   .name = SCC_DRIVER_NAME,
		   .of_match_table = scc_dt_ids,
		   },
	.probe = scc_dev_probe,
	.remove = scc_dev_remove,
	.suspend = scc_suspend,
	.resume = scc_resume,
	.id_table = scc_platform_ids,
};

module_platform_driver(imxscc_driver);
